...
const generatedPaths = {
  ...
  asanaGatekeeper: join(root, "packages/gatekeeper-asana", generatedName),
  quickBooksGatekeeper: join(root, "packages/gatekeeper-quickbooks", generatedName),
  awsGatekeeper: join(root, "packages/gatekeeper-aws", generatedName),
...
const requiredPaths = [
  ...
  "workers.asanaGatekeeper.name",
  "workers.quickBooksGatekeeper.name",
  "workers.awsGatekeeper.name",
  "access.issuer",
  "access.audience",
  "access.admins",
  "aiGateway.enabled",
...
export function validateConfig(config) {
  ...
  if (!Array.isArray(config.integrations.aws.regions) || !config.integrations.aws.regions.length ||
      !config.integrations.aws.regions.every((region) => typeof region === "string" && /^[a-z]{2}-[a-z]+-\d$/.test(region))) {
    throw new Error("AWS regions must be a non-empty array of region names.");
  }
...
export function generateConfigs(config, bases) {
  ...
  const asanaGatekeeper = structuredClone(bases.asanaGatekeeper);
  const quickBooksGatekeeper = structuredClone(bases.quickBooksGatekeeper);
  const awsGatekeeper = structuredClone(bases.awsGatekeeper);
  ...
  workshop.services = [
    ...
    { binding: "GATEKEEPER_ASANA", service: config.workers.asanaGatekeeper.name, entrypoint: "GatekeeperVendor" },
    { binding: "GATEKEEPER_QUICKBOOKS", service: config.workers.quickBooksGatekeeper.name, entrypoint: "GatekeeperVendor" },
    { binding: "GATEKEEPER_AWS", service: config.workers.awsGatekeeper.name, entrypoint: "GatekeeperVendor" },
  ];
  ...
  return { workshop, context, customGatekeeper, asanaGatekeeper, quickBooksGatekeeper, awsGatekeeper, ...(errorReporter && { errorReporter }) };
}
...
function build(config) {
  ...
  run(["--dir", "packages/gatekeeper-asana", "run", "build"]);
  run(["--dir", "packages/gatekeeper-quickbooks", "run", "build"]);
  run(["--dir", "packages/gatekeeper-aws", "run", "build"]);
  if (config.errorReporting.enabled) {
...
async function main() {
  ...
  const generated = generateConfigs(config, {
    ...
    asanaGatekeeper: await readJsonc(join(root, "packages/gatekeeper-asana/wrangler.jsonc")),
    quickBooksGatekeeper: await readJsonc(join(root, "packages/gatekeeper-quickbooks/wrangler.jsonc")),
    awsGatekeeper: await readJsonc(join(root, "packages/gatekeeper-aws/wrangler.jsonc")),
  ...
  try {
    ...
    run(["exec", "wrangler", "deploy", "--config", generatedName, ...deployArgs], join(root, "packages/gatekeeper-asana"));
    run(["exec", "wrangler", "deploy", "--config", generatedName, ...deployArgs], join(root, "packages/gatekeeper-quickbooks"));
    run(["exec", "wrangler", "deploy", "--config", generatedName, ...deployArgs], join(root, "packages/gatekeeper-aws"));
    run(["exec", "wrangler", "deploy", "--config", generatedName, ...deployArgs],

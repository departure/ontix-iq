...
const validConfig = {
  ...
  workers: {
    ...
    asanaGatekeeper: { name: "acme-cloudflare-os-asana" },
    quickBooksGatekeeper: { name: "acme-cloudflare-os-quickbooks" },
    awsGatekeeper: { name: "acme-cloudflare-os-aws" },
...
async function baseConfigs() {
  return {
    ...
    asanaGatekeeper: await baseConfig("../packages/gatekeeper-asana/wrangler.jsonc"),
    quickBooksGatekeeper: await baseConfig("../packages/gatekeeper-quickbooks/wrangler.jsonc"),
    awsGatekeeper: await baseConfig("../packages/gatekeeper-aws/wrangler.jsonc"),
    errorReporter: {
      name: "error-reporter",
...
test("generates Access-mode Workshop, Context, and custom Gatekeeper configs", async () => {
  ...
  assert.deepEqual(generated.workshop.services, [
    ...
    { binding: "GATEKEEPER_ASANA", service: "acme-cloudflare-os-asana", entrypoint: "GatekeeperVendor" },
    { binding: "GATEKEEPER_QUICKBOOKS", service: "acme-cloudflare-os-quickbooks", entrypoint: "GatekeeperVendor" },
    { binding: "GATEKEEPER_AWS", service: "acme-cloudflare-os-aws", entrypoint: "GatekeeperVendor" },
  ]);
